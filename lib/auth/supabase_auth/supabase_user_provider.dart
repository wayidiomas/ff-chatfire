...
class ChatfireDevSupabaseUser extends BaseAuthUser {
  ChatfireDevSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> chatfireDevSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChatfireDevSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
